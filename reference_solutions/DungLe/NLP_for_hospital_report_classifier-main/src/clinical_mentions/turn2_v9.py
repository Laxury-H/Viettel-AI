#!/usr/bin/env python3
"""D2-V9 deterministic CPU inference for the upgraded Viettel AI Race task.
...
Adapted from Laxury-H/Viettel-AI v4.0 at commit
1c003144c127f9e9ac641519ae448a926b2ba8c1.
"""
...
from .turn2_knowledge_base import (
    DIAGNOSES,
    LAB_TEST_PATTERNS,
    MEDICATIONS,
    OFFICIAL_TYPES,
...
def run(input_dir: Path, output_dir: Path, zip_path: Path) -> dict:
    ...
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        for output_file in sorted(output_dir.glob("*.json"), key=natural_key):
            info = zipfile.ZipInfo(f"output/{output_file.name}", (1980, 1, 1, 0, 0, 0))
